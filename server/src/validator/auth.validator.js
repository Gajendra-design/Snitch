...
export const registerValidator = [
    body('username')
        ...
        .trim()
        .notEmpty().withMessage('username cannot be empty').bail()   //noEmpty ye dektha hai client side me input fild me user ne value di hai ya nahi
...
export const loginValidator = [
    ...
    body("password")
        ...
        .exists().withMessage('password field is missing').bail()
        .notEmpty().withMessage('password cannot be empty').bail()
        .isString().withMessage('password must be in text formate').bail()
        .custom(value=>value.trim().length>0)
        .withMessage('password cannot be only black sapces').bail()
        .isStrongPassword().withMessage('password must be atlest 8 charcter loong only also include atleast one small case, one upper case, one number and one special charcter')
]